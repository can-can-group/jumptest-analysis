...
The API runs at a base URL (e.g. `https://your-api.example.com` or behind a reverse proxy at `https://your-domain.com/arge/`). Main areas:
...
| **Email** | `POST /jump-tests/{id}/send-link` sends an email with the viewer link (optional body `{ "email": "override@example.com" }`). Requires SMTP env vars. In production, email links route through the main website gateway, which embeds the viewer via iframe. |
| **Proxy** | Set `BASE_PATH` in `.env` when serving behind a reverse proxy (e.g. `/arge`). All internal links and API calls use this prefix automatically. |
